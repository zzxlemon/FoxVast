            ...
            [&](const std::string& p, const std::string& alias) {
                ...
                if (namesIt != bareNamesByPath.end()) {
                    ...
                    for (const auto& bare : namesIt->second) {
                        auto fIt = parsedFunctions.find(baseNs + "." + bare);
                        if (fIt == parsedFunctions.end()) continue;
                        std::string key = alias + "." + bare;
                        if (parsedFunctions.find(key) != parsedFunctions.end()) continue;
                        Function copy = fIt->second;
                        copy.name = key;
                        parsedFunctions[key] = std::move(copy);
                    }
                }
