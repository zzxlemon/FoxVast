...
void Gc::markFromRoots() {
...
#elif defined(__linux__)
    {
    // Linux: obtain the thread's stack bounds via pthread_getattr_np, then
    // conservatively scan from the current SP up to the stack top.
    ...
    uintptr_t sp;
    __asm__("mov %%rsp, %0" : "=r"(sp));
    uintptr_t low = reinterpret_cast<uintptr_t>(stackAddr);
    uintptr_t high = (stackSize > 0) ? low + stackSize : 0;
    if (high > low && low != 0 && sp >= low) {
        uintptr_t end = high - 0x200;
        for (uintptr_t p = sp & ~(sizeof(uintptr_t) - 1);
             p + sizeof(uintptr_t) <= end; p += sizeof(uintptr_t)) {
            uintptr_t w = *reinterpret_cast<uintptr_t*>(p);
