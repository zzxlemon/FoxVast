...
static LRESULT CALLBACK fgHotkeyWndProc(HWND hwnd, UINT msg,
    ...
    if (it != g_hwndIndex.end()) {
        FGWindow& fgw = FG::windows[it->second];
        if (fgw.prevWndProc) {
            return CallWindowProc(reinterpret_cast<WNDPROC>(fgw.prevWndProc), hwnd, msg, wParam, lParam);
        }
    }
...
#endif // _WIN32 (fgHotkeyWndProc + keyNameToVk)
...
Value FG::simulate_real_click(const std::vector<Value>& args) {
    ...
    throw std::runtime_error(
        "simulate_real_click: OS-level clicks are not supported on this platform");
#endif
}
...
Value FG::register_hotkeys(const std::vector<Value>& args) {
    ...
    HWND hwnd = glfwGetWin32Window(fgw.window);
    if (!fgw.prevWndProc) {
        fgw.prevWndProc = reinterpret_cast<void*>(
...
#else
    throw std::runtime_error(
        "register_hotkeys: global hotkeys are not supported on this platform");
#endif
}
